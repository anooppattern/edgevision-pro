# -*- coding: utf-8 -*-
"""EdgeVision — Retail & QSR · Intel Template.
A simplified, non-technical, outcome-led 30-min pitch deck (16:9)."""
import html, os

OUT = "/home/user/edgevision-pro/deck/EdgeVision-Retail-QSR-IntelTemplate.html"
IMG = "../assets/images"
LOGO=('<img class="intel-logo intel-logo--blue" src="{i}/intel-logo.png" alt="intel"/>'
      '<img class="intel-logo intel-logo--white" src="{i}/intel-logo-white.png" alt="intel"/>').format(i=IMG)
CORNER='<img class="intel-corner" src="{i}/intel-logo-white.png" alt="intel"/>'.format(i=IMG)
COVERLOGO='<img class="cover__intel-logo" src="{i}/intel-logo-white.png" alt="intel"/>'.format(i=IMG)
...
def slide(ctx, body, cls=""):
    _n[0]+=1
    return '<section class="slide {cls}">{lg}{c}<div class="slide__body">{b}</div>{f}</section>'.format(
        cls=cls,lg=LOGO,c=_chrome(ctx),b=body,f=_foot(_n[0]))
...
def statement(ctx, eye, lead, sub, bg):
    ...
    return ('<section class="slide slide--statement"><img class="slide__bg" src="{img}/{bg}" alt=""/>'
        '<div class="divveil"></div>{lg}{c}<div class="slide__body">{b}</div>{f}</section>').format(
        img=IMG,bg=bg,lg=LOGO,c=_chrome(ctx),b=body,f=_foot(_n[0]))
...
def console(stats, hi, alerts, cams):
    DOTB='<svg class="cdot" viewBox="0 0 10 10" width="7" height="7"><circle cx="5" cy="5" r="4.5" fill="#0054AE"/></svg>'
...
S.append('<section class="slide slide--cover"><img class="slide__bg" src="{img}/ev-cover.png" alt=""/>'
  ...
  '<ul class="cover__tags"><li><svg class="cdot" viewBox="0 0 10 10" width="7" height="7"><circle cx="5" cy="5" r="4.5" fill="#00C7FD"/></svg> EDGE-FIRST</li><li><svg class="cdot" viewBox="0 0 10 10" width="7" height="7"><circle cx="5" cy="5" r="4.5" fill="#00C7FD"/></svg> PRIVACY BY DESIGN</li>'
  '<li><svg class="cdot" viewBox="0 0 10 10" width="7" height="7"><circle cx="5" cy="5" r="4.5" fill="#00C7FD"/></svg> INTEL POWERED</li></ul>'
...
S.append('<section class="slide slide--closing"><img class="slide__bg" src="{img}/ev-closing.png" alt=""/>'
  '<div class="closing__veil"></div>{cor}<div class="closing__inner">'
  ...
  '<span>© 2026 PATTERN AI LABS · INTEL EDGE AI PARTNER</span></div></div></section>'.format(img=IMG,mk=mark("mark--lg"),cor=CORNER))

CSS=open("/home/user/edgevision-pro/deck/deck.css").read()+"\n"+open("/home/user/edgevision-pro/deck/intel.css").read()
doc=('<!doctype html><html lang="en"><head><meta charset="utf-8"/>'
     '<title>EdgeVision — Retail & QSR · Intel Template</title>'
